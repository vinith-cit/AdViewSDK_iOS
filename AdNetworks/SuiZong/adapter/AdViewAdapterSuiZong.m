...
- (void)getAd {
	Class SuiZongAdOnClass = NSClassFromString (@"KOpenAPIAdView");
	
	if (nil == SuiZongAdOnClass) {
		[adViewView adapter:self didFailAd:nil];
		AWLogInfo(@"no SuiZong lib, can not create.");
		return;
	}
	
	[self updateSizeParameter];
	
	KOpenAPIAdView *adView = [SuiZongAdOnClass requestOfSize: self.sSizeAd withDelegate:self 
												  withAdType:KOPENAPIADTYPE_SUIZONG];
	if (nil == adView) {
		[adViewView adapter:self didFailAd:nil];
		return;
	}
...
- (void)stopBeingDelegate {
  KOpenAPIAdView *adView = (KOpenAPIAdView *)self.adNetworkView;
	AWLogInfo(@"--SuiZong stopBeingDelegate--");
  if (adView != nil) {
	  adView.delegate = nil;
